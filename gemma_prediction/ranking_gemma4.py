...
from gemma_prediction.llm_postprocess import (
    apply_base_rate_reranking,
    print_comparison,
)
...
def main():
    ...
    parser.add_argument("--use_base_rate_cap", action="store_true",
                        help="After sliding window voting, re-rank DAs by vote "
                             "fraction and mark only the top base_rate*n_das as "
                             "important (base rate from training set). Applied "
                             "before the min-run filter. Adds pred_reranked "
                             "column to output CSV.")
    ...
    # ── base-rate re-ranking (optional post-processing) ───────────────────────
    reranked_metrics: dict = {}
    if args.use_base_rate_cap:
        print(f"\nApplying base-rate re-ranking (base_rate={base_rate:.4f}) …",
              flush=True)
        pred_rows, reranked_metrics = apply_base_rate_reranking(
            pred_rows=pred_rows,
            base_rate=base_rate,
            min_important_run=args.min_important_run,
            min_unimportant_run=args.min_unimportant_run,
        ...
        print_comparison(metrics, reranked_metrics, base_rate)
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        json.dump({
            ...
            "use_base_rate_cap":   args.use_base_rate_cap,
            **metrics,
            **reranked_metrics,
            **ceiling,
        }, f, indent=2)
