...
def run_loocv(
    ...
    for fold_idx, test_fname in enumerate(fnames):
        ...
        y_pred = predict_transcript(model, tok, ts, device, hp.threshold)
    ...
    for i, hp in enumerate(grid):
        ...
        try:
            results, _ = run_loocv(
                ...
                hp=hp,
                device=device,
                verbose=True,       # suppress per-fold noise during sweep
...
# Useful cmds:
# grep pooled_f1_imp ./*
# grep mean_fold_f1_imp cnn_output/grid_search_*.csv | sort -t, -k<col> -n -r | head

# TODO: Need to add balanced f1 as a metric, to compare with stephen's
#       ^ Done: pooled_f1_balanced + mean_fold_f1_balanced now in all outputs